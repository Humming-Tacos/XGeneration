...
def render(song_id, line, generate_midi=False):
    data = np.load(
        r'C:\Users\86152\Desktop\Research\Accompaniment-Generation-via-Music-Representation-Learning-main\POP09'
        r'-PIANOROLL-4-bin-quantization' + "\\" + str(song_id).zfill(3) + '.npz')
    chord = data['chord']
    melody = data['melody']
    bridge = data['bridge']
    piano = data['piano']
    ...
    if generate_midi:
        ...
        midi.write(
            os.path.join('./data files/POP909 4bin quntization/POP-909_recon/', str(song_id) + '_accompaniment.mid'))

    return melody_record, pr_matrix_mix, chord_record


def build_phrase_dataset():
    segmentation_root = r'C:\Users\86152\Desktop\Research\Accompaniment-Generation-via-Music-Representation-Learning-main\AccoMontage\AccoMontage\data files\POP909 hierarchical-structure-analysis'
    df = pd.read_excel(r'C:\Users\86152\Desktop\Research\Accompaniment-Generation-via-Music-Representation-Learning'
                       r'-main\AccoMontage\AccoMontage\data files\POP909 4bin quntization\index.xlsx')
    full_melody = []
    full_acc = []
    full_chord = []
    for song_id in tqdm(range(1, 910)):
        ...
        data = np.load(r'C:\Users\86152\Desktop\Research\Accompaniment-Generation-via-Music-Representation-Learning'
                       r'-main\POP09-PIANOROLL-4-bin-quantization' + "\\" + str(song_id).zfill(3) + '.npz')
        melody = data['melody']
...
def build_song_dataset():
    ...
    for song_id in tqdm(range(1, 910)):
        ...
        beat_shift = melody_start_measure * 4 - melody_start_note // 4
        data = np.load(
            './data files/POP909 4bin quntization/POP09-PIANOROLL-4-bin-quantization/' + str(song_id).zfill(3) + '.npz')
        melody = data['melody']
        chord = data['chord']
        bridge = data['bridge']
        piano = data['piano']
