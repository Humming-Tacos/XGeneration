...
def adjust_chord(input_path, output_path):
    ...
    for i in range(len(nn)):
        if i == 0:
            continue
        if nn[i][0] != nn[i - 1][1] and i % 3 == 0:
            for j in range(3):
                nn.append([nn[i - 1][1], nn[i][0], nn[i + j][2], 90])
...
def export_music(score, chord_list, gap_list, filename, midi_tempo):
    ...
    output_filename = '/root/temporary/' + filename + '_chord.mid'
...
def generate_chord(midi_path):
    # Build model
    model = build_model(weights_path='/root/task2_explicit/weights.hdf5')
    ...
    for idx in trange(len(data_corpus)):
        ...
        res = export_music(score, chord_list, gap_list, filename, midi_tempo)
    return res
