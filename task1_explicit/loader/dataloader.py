    ...
    def load(self, directory):
        ...
        if self.datasetName == "Nottingham":
            for midi_file in path:
                self.midi_files.append(
                    {"name": (midi_file.split("."))[0], "raw": pyd.PrettyMIDI(directory + midi_file)})
                if total % 1000 == 0:
                    ...
                    if len(chord_set) == 0:
                        ...
                        if note.start == chord_time[0] and note.end == chord_time[1]:
                            ...
                            if last_time < chord_time[0]:
                                self.midi_files[i]["chords"].append(
                                    {"start": last_time, "end": chord_time[0], "chord": "NC"})
                            self.midi_files[i]["chords"].append(
                                {"start": chord_time[0], "end": chord_time[1], "chord": cl.note2name(chord_set)})
                            ...
                            if chord == prev:
                                ...
                                if prev == "NC":
                                    ...
                                    for i_note in i_notes:
                                        i_note = pyd.Note(velocity=100, pitch=i_note,
                                                          start=time_shift,
                                                          end=time_shift + local_duration * self.minStep)
                                        chords.notes.append(i_note)
                                    prev = chord
                                    time_shift += local_duration * self.minStep
                                    local_duration = 1
...
class DataLoader:
    ...
    def start_new_epoch(self, batch_size=64):
        ...
        self.validate_batches = np.split(self.validate_set,
                                         range(batch_size, self.validate_set.shape[0] // batch_size * batch_size,
                                               batch_size))
